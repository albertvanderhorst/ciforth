...
: VERTICAL-FRAME 20 0 DO BLUE @ VV I 50 * + 81 + LC! 2 +LOOP ;
 ...
 DECIMAL
: LEAVE-BLOCK BLK @ IF 1023 IN ! THEN ;
...
: PARSE WORD $@ ;     : ($) R $@ DUP 1+ R> + >R ;

: S" [CHAR] " PARSE 2DUP ( fig: already HERE) 1+ ALLOT DROP ;

: C" COMPILE ($) [CHAR] " WORD C@ 1+ ALLOT ; IMMEDIATE
...
: COLLECT-REST ( OF LINE, MAY CONTAIN SPACES ## MEANS EMPTY)
 DLM WORD ## $@= IF NILL ELSE HERE $@ $SAVE THEN , ;
