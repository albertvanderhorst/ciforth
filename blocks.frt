...
( BNF PARSER                                   ch&ch )
0 VARIABLE SUCCESS
( Parses, remember parse pointers/ not parses: skip this word)
: <BNF  SUCCESS @ IF R> IN @ >R DP @ >R >R ELSE R> DROP THEN ;
: BNF>  SUCCESS @ IF R> R> DROP R> DROP >R ELSE
 R> R> DP ! R> IN ! >R  THEN ;
: | SUCCESS @ IF R> R> R> 2DROP DROP ELSE
 R> R> R> 2DUP >R >R IN ! DP ! 1 SUCCESS ! >R  THEN ;
: BNF: [COMPILE] : SMUDGE COMPILE <BNF 1 SUCCESS ! ;
IMMEDIATE
