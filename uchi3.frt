...
\ We want this:    MAX-LINES 2 CELLS ARRAY LINES
CREATE LINES \ Not yet

\ Prevent silly static array in the executable.
\ Call this before using LINES .
: ALLOCATE-LINES
"MAX-LINES 2 CELLS ARRAY LINES-NEW" EVALUATE
"'LINES-NEW 'LINES 3 CELLS MOVE"    EVALUATE
;
...
\ and leave pointers to those strings at POSITION .
: SAVE-AT-POS
    LINES >R
    $, R@ CELL+ !    $, R@ !
    RDROP ;
...
\ Print both fields of the LINE in ``LINES''
: PRINT-FULL
            LINES @+ $@ TYPE
            ^I EMIT
...
\ Print only the second one of the fields of the LINE .
: PRINT-HALF
            LINES CELL+ @ $@ TYPE
;

\ Output the whole lot of ``LINES''
: PUT-ALL
        0 PRINT-FULL
        LINE# @ 1 ?DO
...
\ Input of all records into the ``LINES'' array.
: GET-ALL
    0 LINE# !
    BEGIN
        GET-RECORD
