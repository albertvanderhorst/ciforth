...
: .FOR DUP RE-COMPILED = IF
    DUP @ 'FORTRACK = IF CELL+ ELSE CR "^ : MATCHING AT START" TYPE THEN
THEN ;
...
: .A* DUP @ 'ADVANCE* = IF CR "* OF " TYPE      CELL+ .CHAR THEN ;
: .A+ DUP @ 'ADVANCE+ = IF CR "+ OF " TYPE      CELL+ .CHAR THEN ;
\ : .A() DUP @ 'ADVANCE() = IF CELL+ @+ DUP 1 > IF
\     1 AND IF ")END" ELSE "START(" THEN CR TYPE  _
\ THEN DROP ;
: .RE .FOR BEGIN DUP @ WHILE .DOLLAR  .EXACT .CHAR .A? .A* .A+ REPEAT DROP ;
