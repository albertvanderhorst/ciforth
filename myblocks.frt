...
800 10FF 4 2FAMILY, CALLO, CALLFARO, JMPO, JMPFARO,             
                                                                                                                                                                                                                                                                ( POST-IT/FIX-UP 8086 ASSEMBLER , POSTLUDE AvdH HCCFIG HOLLAND) HEX VOCABULARY ASSEMBLER IMMEDIATE                              ' ASSEMBLER CFA ' ;CODE 8 + !        ( PATCH ;CODE IN NUCLEUS ) : CODE ?EXEC CREATE [COMPILE] ASSEMBLER !CSP ; IMMEDIATE        : C; CURRENT @ CONTEXT ! ?EXEC ?CSP SMUDGE ; IMMEDIATE          : NEXT                                                               LODS, W1|                                                       MOV, W| F| R| BX| AX1|                                          MOV, W| F| BX1| DX|                                             INCX, DX|                                                       JMPO, D0| [BX]                                              ;                                                               CODE TEST NEXT  C;                                                                                                               9      MOV, W| F| BX1| DX|                                      9      MOV, W| F| BX1| DX|                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    
...
( BASIC STYLE MINI EDITOR - FORTH DIM iii/2 @ SHAPIN)           HEX : TEXT HERE C/L 1+ BLANKS WORD HERE PAD C/L 1+ CMOVE ;      : LINE DUP FFF0 AND 17 ?ERROR SCR @ (LINE) DROP ;               : -MOVE LINE C/L CMOVE UPDATE ;                                 : P 1 TEXT PAD 1+ SWAP -MOVE ; DECIMAL                          ( Usage : to change line 1 of screen 3 )                        ( 3 SCR ! 1 P <CONTENT> )                                       : CLEAN BLOCK B/BUF OVER + SWAP DO                                I C@ 0= IF BL I C! THEN  LOOP ;                               : THRU 1+ SWAP DO ." LOADING " I . I LOAD LOOP ;                : L-S SCR @ LIST ; : LO-S SCR @ LOAD ;                          : C-S SWAP BLOCK SWAP BLOCK B/BUF CMOVE UPDATE FLUSH ;          : BIOSI BIOS DROP DROP DROP DROP DROP ;  ( Ignore result)       : MODE 0 0 0 16 BIOSI ;                                         : DISK-INIT 0 0 0 0 19 BIOSI ;                                  : LIST BASE @ 10 - 24 ?ERROR LIST ;                             ( MINI EDITOR FOR MSDOS ) HEX                                   B800 CONSTANT VID   084 CONSTANT LL   19 CONSTANT VH            LL CONSTANT VW    VH VW * CONSTANT VL                           : A-L SCR @ (LINE) ;                                            : VA ( I - VS,VO ) DUP + VID SWAP ;                             : >V ( $, OFFSET - )                                            SWAP 0 DO                                                        OVER I + C@ 700 OR   OVER I + VA L!                            LOOP DROP DROP ;                                                : V> ( BUF -LEN OFFSET- )                                       SWAP 0 DO OVER I +   OVER I + VA                                 L@ SWAP C!                                                     LOOP DROP DROP ;                                                : PAGE PAD VL 2DUP BLANKS 0 >V ;                                : PG PAD 10 VW * 2DUP BLANKS 0 >V ;                              DECIMAL                                                        HEX                                                             : GET-S 10 0 DO I A-L I VW * >V LOOP ;                          : PUT-S 10 0 DO I A-L I VW * V> LOOP UPDATE ;                   : GET-L PAD LL ROT LL * >V ;                                    : PUT-L PAD LL ROT LL * V> ;                                    ( 19 CONSTANT PB )   13 CONSTANT HW                             : PUSH-D 10 SWAP DO I 1+ PUT-L I GET-L LOOP ;                   : PUSH-DOWN VH SWAP DO I 1+ PUT-L I GET-L LOOP ;                : DEL-L DUP PUT-L VH  GET-L PUSH-DOWN ;                         : PUSH-UP VH DO I PUT-L I 1+ GET-L -1 +LOOP ;                   : UDL-L DUP PUSH-UP VH PUT-L GET-L ;                            : PUSH-U HW DO I PUT-L I 1+ GET-L -1 +LOOP ;                    : UFL-L DUP PUSH-U VH 1 - PUT-L GET-L ;                         : DUP-L DUP DEL-L DUP UFL-L UDL-L ;                                                                                              DECIMAL                                                        HEX ( CURSOR E.D.)                                              0 VARIABLE CURSOR VW CONSTANT LL                                 : CURL CURSOR @ LL / ; : CP CURSOR @ LL MOD  ;                  : BIOS-CURSOR CURSOR @ VW /MOD 100 * + ;                       : SET 200 0 0 BIOS-CURSOR 10 BIOS DROP DROP DROP DROP DROP ;    : MOVE-CURSOR   ( WORD STAR)                                    DUP ^D = IF  1 ELSE   DUP ^E = IF 0 LL - ELSE                   DUP ^I = IF  8 ELSE   DUP ^M = IF LL CP - ELSE                  DUP ^S = IF -1 ELSE   DUP ^X = IF LL ELSE                       0    THEN THEN THEN THEN THEN THEN CURSOR +! ;                  : DELSTORING DUP ^Y  = IF CURL DEL-L ELSE                             DUP ^P = IF CURL UDL-L ELSE                                     DUP ^U = IF CURL UFL-L                                          THEN THEN THEN ;                                          : DUP-L CURL DUP-L ;                                              DECIMAL                                                       HEX                                                             : PAD-B PAD VW BLANKS  ;                                        : GET-R PAD VW CP - CURL VW * CP + >V ;                         : GET-P PAD    CP   CURL VW *      >V ;                         : PUT-R PAD VW CP - CURL VW * CP + V> ;                         : RUBOUT-M PUT-R MINUS CURSOR +! GET-R ;                        : INSERT-M PUT-R DUP CURSOR +! GET-R MINUS CURSOR +! ;          : RUBOUT 1 RUBOUT-M ; : INSERT 1 INSERT-M ;                     : RUB-C PAD-B RUBOUT SET ;                                      : DEL-C 1 CURSOR +! RUB-C ;                                     : INS-C INSERT ;                                                : EOL PAD-B GET-R ;  : FOL PAD-B GET-P ;                        : SPL DUP-L PAD-B PUT-R CURL 1+ GET-L EOL ;                     : JOL PAD-B CURL 1+ PUT-L GET-R CURL 1+ PUSH-D ;                DECIMAL
                                                                                                                         HEX                                                              0 VARIABLE I-MODE                                              : INSELETING                                                          DUP ^H = IF RUB-C ELSE                                          DUP ^G = IF DEL-C ELSE                                          DUP ^V = IF I-MODE 1 TOGGLE ELSE                                THEN THEN THEN ;                                          : JOINITTING                                                          DUP ^J = IF JOL   ELSE                                          DUP ^O = IF SPL   ELSE THEN THEN ;                        : EM-C EMIT 1 CURSOR +! ;                                       : PRINT ( C --C . Print it if printable)                          DUP 1F > IF DUP 7F < IF                                         I-MODE @   IF INS-C THEN  DUP EM-C                              THEN THEN ;                                                   DECIMAL                                                         ( DISPATCHER )    HEX                                           : AT-END VH LL * CURSOR ! SET ;                                 : DEBUG CURSOR @ AT-END ^ CURSOR ! ;                            : EXITING KEY &Q - IF PUT-S THEN ;                              : ROUTE BEGIN KEY                                               PRINT DELSTORING                                                INSELETING JOINITTING                                           WORDING MOVE-CURSOR SET                                         ( DEBUG )                                                       ESC = UNTIL ;                                                   : E-S  ( EDIT CURRENT SCREEN )                                  0 CURSOR ! SET   PG                                             GET-S ROUTE EXITING  AT-END ;                                   :  EDIT SCR ! E-S ;                                             DECIMAL                                                                                                                          ( Finding the next word)                                        : BL? VA L@ $FF AND BL = ;                                     : BOUNDARY? BL? 0= OVER 1 - BL? AND ;                           : >SEARCH BEGIN 1 + DUP BOUNDARY? UNTIL  ;                      : <SEARCH BEGIN 1 - DUP BOUNDARY? UNTIL  ;                      : NEXT-W CURSOR @ >SEARCH CURSOR ! ;                            : BACK-W CURSOR @ <SEARCH CURSOR ! ;                            : DEL-W  CURSOR @ NEXT-W SPL                                       CURSOR ! SET SPL CURL 1+ DEL-L JOL ;                         : VTYPE CURSOR @ >V ;                                           : GET-W VH 1 - PUT-L PAD VW -TRAILING 1+ ;                      : UFW GET-W INSERT-M DROP GET-W VTYPE ;                         : WORDING                                                         DUP ^F = IF NEXT-W ELSE   DUP ^A = IF BACK-W ELSE               DUP ^T = IF DEL-W ELSE    DUP ^Z = IF UFW ELSE                THEN THEN THEN THEN ;                                           108 CONSTANT SCRATCH HEX                                        : FROM-BIOS 100 /MOD VW * + CURSOR ! ;                          : GET 300 0 0 0 10 BIOS DROP FROM-BIOS  DROP DROP DROP ;        : EMPTY SCRATCH BLOCK B/BUF BLANKS ;                            : P ( AT-END) GET ROUTE ( EMPTY) CURL PUT-L                       ( PAD SCRATCH BLOCK LL CMOVE )                                   AT-END CR PAD 50 1 - -TRAILING TYPE SPACE                       PAD TIB @ 50 CMOVE 0 IN !                                    ; ( IMMEDIATE  DOESN'T WORK VIA S)                              DECIMAL
